...
%% IED with and without

%figurefull;

subjects = TsubRes.Subject( TsubRes.Role == 'TREAT'); % treatments
...
for ir = 1:Nsubj
    subject = subjects(ir);
    x = fevalc(  Tiedf.Signal( Tiedf.Subject == subject & Tiedf.HasR )   ); % signals from one subject
    if ~isempty(x), ied1(ir,:) = nanmean(x,1); end;
    x = fevalc(  Tiedf.Signal( Tiedf.Subject == subject & ~Tiedf.HasR )   ); % signals from one subject
    if ~isempty(x), ied2(ir,:) = nanmean(x,1); end;
end
...
%% IED with and without with  confidence
...
ied=NaN(Nsubj,5000); iedsems=NaN(Nsubj,5000);
%ied2=NaN(Nsubj,5000);
for ir = 1:Nsubj
    subject = subjects(ir);
    x = fevalc(  Tiedf.Signal( Tiedf.Subject == subject & Tiedf.HasR )   ); % signals from one subject
...
function stats = plotDotBoxplot_CXvsTREAT(Tsub,feature,labels,w,h)
